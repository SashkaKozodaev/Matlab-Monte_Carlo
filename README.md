...
проводящего состояния в диэлектрическое с учётом перескоков электронов
на соседние атомы и кулоновского отталкивания на узле. Гамильтониан
Хаббарда имеет вид:

$$ H = -t\sum_i(a_i^+a_{i+1} + a_{i+1}^+a_i) + U\sum_in_i^2 + V\sum_i n_in_{i+1} - \mu\sum_in_i $$ (1)

<img src="https://latex.codecogs.com/svg.latex?H = -t\sum_i(a_i^+a_{i+1} + a_{i+1}^+a_i) + U\sum_in_i^2 + V\sum_i n_in_{i+1} - \mu\sum_in_i" />
...
new конфигураций, равные произведе-нию весов каждого из 4 черных квадратов. На основании отношения wnew/wold
принимается решение о смене конфигурации
>
...
квадратов, окружающих некоторый белый квадрат. Эта процедура
...
Из полученных данных следует,что при увеличении параметра U скорость сходимости увеличивается,
